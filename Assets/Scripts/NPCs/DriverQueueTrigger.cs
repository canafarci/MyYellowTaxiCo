...
namespace Taxi.NPC
{
    public class DriverQueueTrigger : MonoBehaviour
    {
        ...
        {
            while (_inventories[other].GetItem(_queue.ItemType))
            {
                StackableItem item = _inventories[other].GetItem(_queue.ItemType);
